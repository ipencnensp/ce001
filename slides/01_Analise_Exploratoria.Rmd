...
- Amostragem:
  ...
  - Conglomerados
  - Sistemática
- Estatística Descritiva (consistência e interpretações iniciais).
- Inferência estatística (baseado na amostra extrapolar para a população).
...
- A partir de um conjunto de dados coletado, a questão é:
  - Como extrair informações a respeito de uma ou mais características de
    interesse?
...
```{r, comment=NA}
# Carregando os dados
dados <- read.table("questionario.txt", header = TRUE)
...
- Cuidado deve ser tomado com representações visuais pois um gráfico
  desproporcional pode gerar interŕetações distorcidas
- Os principais são:
